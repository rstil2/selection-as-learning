...
The empirical centerpiece of this paper is a cross-cancer validation in Figure~3: the theoretical PAC bound $\varepsilon^*(N_e, V_A)$ derived in Theorem~\ref{thm:pac} correlates with observed resistance durability at $r = 0.96$ ($p < 0.0001$) across nine cancer types, and the FTNS convergence rate $V_A/\bar{W}$ correlates with measured selection coefficients at $r = 0.999$ across twenty cancer types. These quantitative predictions---derived entirely from first principles using only quantities measurable in clinical sequencing pipelines---confirm that the evolutionary sample complexity threshold $N_e^*$ is a real, predictive clinical parameter. The following subsections develop the theoretical framework that predicts and explains these results.
