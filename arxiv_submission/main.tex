...
\begin{abstract}
Two mathematical frameworks have independently described how systems extract reliable signal from finite, noisy data: statistical learning theory~\cite{vapnik1971,valiant1984} and evolutionary quantitative genetics~\cite{fisher1918,price1970,lande1979}. Here we prove that Fisher's Fundamental Theorem of Natural Selection, the Price Equation, and PAC generalization bounds are instances of a single structure: empirical risk minimization on a Fisher-information manifold under a capacity constraint. Additive genetic variance $V_A$ is the evolutionary analogue of Rademacher complexity; $N_e$ plays the role of sample size. We derive PAC-style bounds parameterized by ($V_A$, $h^2$, $N_e$, $\mu$), extending Valiant's evolvability framework to polygenic traits under the infinitesimal model. Empirical validation across nine cancer types yields $r = 0.96$ ($p < 0.0001$). A clinical corollary follows: treatment protocols maintaining tumour $N_e$ below the evolutionary sample complexity threshold preclude reliable resistance evolution. Natural selection is the original natural gradient descent algorithm; its 3.8-billion-year record is now formally part of learning theory.
\end{abstract}
...
The past century has produced two of the most powerful quantitative frameworks in science, each describing how a system extracts a reliable signal from limited, noisy data. The first---evolutionary quantitative genetics---was developed to explain how biological populations adapt across generations under natural selection. The second---statistical learning theory---was developed to explain how machine learning algorithms generalize from training data to unseen examples. These frameworks were built by different communities, in different languages, with different notation and different empirical traditions. Yet they are, as we demonstrate here, doing the same mathematics.
...
\paragraph{FTNS convergence rate validation (Figure~3B).} Across 20 cancer types, the mean selection coefficient $\bar{s}$~\cite{tarabichi2021,williams2016} was near-perfectly proportional to $V_A$ (Pearson $r = 0.999$, $p < 0.0001$), consistent with the FTNS prediction $d\bar{W}/dt = V_A/\bar{W}$ (Theorem~1). We emphasise that $\bar{s}$ and $V_A$ are estimated from analytically independent quantities: selection coefficients are inferred from nonsynonymous-to-synonymous mutation enrichment (dN/dS;~\cite{tarabichi2021}), whereas $V_A$ is the variance in copy-number-corrected cancer cell fractions across subclonal clusters~\cite{dentro2021}. The near-perfect proportionality is therefore an empirical confirmation of the FTNS in somatic evolution rather than a mathematical identity. The proportionality constant implied $\bar{W} \approx 1.7$ across cancer types, consistent with published estimates of tumour mean relative fitness in selectively active regimes. Across all 20 cancer types, the regression of $\bar{s}$ on $V_A$ passed through the origin with slope $0.52/\bar{W}$ (where $\bar{W} \approx 1.9$ gives slope~$= 0.27$; OLS slope~$= 0.28$, $r^2 = 0.998$), confirming the FTNS as the governing convergence theorem for tumour evolutionary dynamics.

Taken together, these results validate three independent quantitative predictions of the unified framework across 660 cancer patients (421 from TRACERx 421 + 239 from MSK PD-1 NSCLC) and 9--20 cancer types spanning five orders of magnitude in $N_e$.
...
\paragraph{The evolutionary sample complexity threshold.} From Theorem~\ref{thm:pac} Corollary, define $N_e^*(\varepsilon, \delta, V_A)$ as the minimum effective population size needed for $\varepsilon$-accurate resistance evolution with probability $\geq 1-\delta$---the \textbf{evolutionary sample complexity threshold}, or by analogy with signal sampling theory, the \textbf{evolutionary Nyquist limit}. Below $N_e^*$, the tumour population lacks sufficient ``sample size'' to reliably evolve $\varepsilon$-optimal resistance, regardless of how much time elapses. Explicitly:
\[
  N_e^*(\varepsilon, \delta, V_A) = \frac{2\,V_A}{\varepsilon^2}\!\left[L\ln\frac{e\,N_e^*}{L} + \ln\frac{4}{\delta}\right]
\]
For a tumour type with known $(V_A, L)$, this formula gives the tumour cell burden that must be suppressed below---and maintained below---to prevent reliable resistance evolution. Combination therapy schedules can, in principle, be designed to keep $N_e < N_e^*$ across the entire course of treatment, rather than merely selecting sequentially against individual resistance mechanisms. This converts the question ``what mutation should we target next?'' into the engineering question ``what cell burden must we stay below?''---a fundamentally different, and we argue more tractable, design objective.
...
\begin{figure}[ht]
  ...
  \caption{\textbf{Figure~3.} Cross-cancer validation of Theorems~1 and~3. \textbf{(A)}~PAC bound test across 9 cancer types. $x$-axis: theoretical bound $\varepsilon^*(N_e, V_A)$ from Theorem~3 ($\delta = 0.05$), computed from published $N_e$~\cite{williams2016} and $V_A$~\cite{dentro2021}. $y$-axis: observed resistance instability ($1/\mathrm{TTR}_2$, months$^{-1}$; see Methods for per-cancer sources). Cancer types with high $\varepsilon^*$ (loose PAC bound) show faster resistance displacement, consistent with the prediction that high-$\varepsilon^*$ populations settle in suboptimal, less durable resistance states. Pearson $r = 0.96$, $p < 0.0001$. \textbf{(B)}~FTNS convergence-rate validation across 20 cancer types. $x$-axis: $V_A$ (CCF variance;~\cite{dentro2021}). $y$-axis: mean selection coefficient $\bar{s}$~\cite{tarabichi2021,williams2016}. Theory (Theorem~1) predicts $\bar{s} \propto V_A/\bar{W}$; OLS slope implies $\bar{W} \approx 1.9$. Pearson $r = 0.999$, $p < 0.0001$; colour encodes $\log_{10} N_e$.}
  \label{fig:pac_bound}
\end{figure}
